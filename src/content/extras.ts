...
export const EXTRA_EXERCISES: readonly Exercise[] = [
    ...
    steps: [
      {
        ...
        choices: ['Une architecture sans hiérarchie thématique et un maillage interne pauvre : le budget de crawl se perd et l’autorité ne remonte pas vers les piliers', 'Le site est trop rapide', 'Il y a trop d’articles', 'Les titres sont trop longs'],
        answer: 0,
  ...
  qcm('mkt-case-3', 4, 'Ta campagne Meta a un CPA de 12 € sur les 7 premiers jours puis 30 € les jours suivants, sans changement. Cause la plus probable ?', ['La fatigue publicitaire : l’audience a trop vu la même création, la fréquence monte et le CTR baisse', 'Meta a changé ses prix', 'Le produit est devenu mauvais', 'Le pixel est cassé'], 'Fréquence > 3-4 et CTR en baisse = fatigue. Un pixel cassé ferait disparaître les conversions, pas doubler le CPA. Remèdes : nouvelles créations, élargissement d’audience, ou pause.'),
  ...
  qcm('ia-case-1', 5, 'Tu veux qu’un modèle produise toujours un JSON valide avec les mêmes champs. Le plus fiable ?', [
    'Imposer un schéma de sortie (structured outputs) quand l’API le propose, sinon un exemple few-shot, une température basse et une validation du JSON côté code', 'Demander poliment', 'Augmenter la température', 'Utiliser un modèle d’image'],
  ...
  vf('ia-llm-2', 4, 'Un score élevé sur un benchmark public garantit de bons résultats sur mes propres tâches.', false, 'Les benchmarks sont publics, donc dans les données d’entraînement ; ils mesurent des tâches génériques. Seul un jeu d’évaluation sur tes cas réels compte.'),
  ...
  qcm('ia-agents-1', 1, 'Que produit le modèle quand il veut utiliser un outil que tu lui as déclaré (function calling) ?', ['Un appel structuré (nom de l’outil + arguments), que ton programme exécute avant de lui renvoyer le résultat', 'Il accède lui-même à tes systèmes, sans passer par ton code', 'Du texte libre décrivant ce qu’il ferait', 'Rien : un modèle ne peut pas agir'], 'Le modèle ne fait que demander ; c’est ton code qui exécute, puis renvoie le résultat dans la conversation. D’où l’importance des garde-fous.', { code: { lang: 'text', src: '{ "tool": "get_ga4_report",\n  "arguments": { "metric": "sessions", "days": 7 } }' } }),
  ...
  qcm('py-concepts-1', 104, 'Pourquoi ce code produit-il une erreur ?', ['Le corps du if n’est pas indenté', 'Il manque des parenthèses au if', 'print n’existe pas', 'clics doit être une chaîne'], 'Après `if …:`, le bloc doit être indenté (4 espaces par convention PEP 8). Python lève une IndentationError.', { code: { lang: 'python', src: 'clics = 120\nif clics > 100:\nprint("bonne journée")' } }),
  ...
  qcm('py-concepts-2', 105, 'Pourquoi ce code produit-il une erreur ?', ['Un tuple est immuable : on ne peut pas modifier un élément', 'Les tuples n’acceptent que des chaînes', 'Il manque un import', 'L’index 0 n’existe pas'], 'TypeError: \'tuple\' object does not support item assignment. Utilise une liste si les valeurs doivent changer.', { code: { lang: 'python', src: 'point = (48.85, 2.35)\npoint[0] = 50' } }),
  ...
  qcm('py-web-1', 103, 'Que renvoie cette ligne ?', ['La liste des éléments <a> ayant la classe "produit"', 'Le premier lien de la page', 'Le texte de la page', 'Une erreur'], 'select() prend un sélecteur CSS et renvoie tous les éléments qui correspondent ; select_one() renverrait le premier.', { code: { lang: 'python', src: 'soup = BeautifulSoup(html, "html.parser")\nliens = soup.select("a.produit")' } }),
  ...
  qcm('web-js-2', 102, 'Qu’affiche ce code ?', ['undefined : au moment du console.log, la réponse n\'est pas encore arrivée', 'Le tableau des produits', 'Une erreur', 'Rien'], 'fetch est asynchrone : le console.log s\'exécute tout de suite, avant les .then(), donc produits vaut encore undefined. Il faut attendre la promesse : placer le console.log dans le dernier .then(), ou utiliser await dans une fonction async.', { code: { lang: 'js', src: 'let produits;\nfetch("/api/produits").then((r) => r.json()).then((d) => { produits = d; });\nconsole.log(produits);' } }),
